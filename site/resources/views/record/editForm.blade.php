...
@section('content')
  <div id="app">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-11"><h4>Edit PPA Inspection Record Form</h4></div>
                ...
                <thead>
                  <td width="33%">
                    <div class="form-group row">
                      ...
                      <div class="col-sm-8">
                        <input type="text" class="form-control" disabled v-model="record.c_order_number">
                      </div>
                    </div>
                    <div class="form-group row">
                ...
                <thead>
                  ...
                  <!-- ========================================== Failure symotom ====================================== -->
                    ...
                    <tr v-for="(cl, index) in record.failureSymptomChecklist">
                      ...
                      <td :colspan="record.machineList.length > 0 ? record.machineList.length : 1" v-if="record.mesurementChecklist.length == 0"></td>
    ...
    var app = new Vue({
      ...
      data: {
        ...
        record: {
          ...
          mesurementChecklist: [
            @foreach($mesurementChecklist as $value)
              {
                c_code: '{{ $value->c_code }}',
                c_rank: '{{ $value->c_rank }}',
                f_max_value: '{{ $value->f_max_value }}',
                f_min_value: '{{ $value->f_min_value }}',
                i_checklist_deleted: '{{ $value->i_checklist_deleted }}',
                i_checklist_id: '{{ $value->i_checklist_id }}',
                i_errorcode_deleted: '{{ $value->i_errorcode_deleted }}',
                i_errorcode_id: '{{ $value->i_errorcode_id }}',
                i_errorcode_type_id: '{{ $value->i_errorcode_type_id }}',
                i_form_id: '{{ $value->i_form_id }}',
          ...
          testSpecificationChecklist: [
            @foreach($testSpecificationChecklist as $value)
              {
                c_code: '{{ $value->c_code }}',
                c_rank: '{{ $value->c_rank }}',
                f_max_value: '{{ $value->f_max_value }}',
          ...
          palletList: [
            @foreach($recordPallet as $value)
              {
                status: {{ $value->i_record_pallet_status }}
              },
            @endforeach
          ],
      ...
      mounted() {
        ...
        @foreach($machineList as $value)
          @php($i = 0)
          @foreach($failureSymptomChecklist as $v)
            @foreach($recordFailure as $val)
        ...
        @foreach($recordFailureDetails as $key => $value)
          this.record.failureSymptomRejectDetail[{{ $key }}] = {
            i_errorcode_id: '{{ $value->i_errorcode_id }}',
            errorcodeIndex: '{{ $key }}',
            value: '{{ $value->c_detail }}'
        ...
        this.record.mesurement.forEach((item, index) => {
              ...
              case 'mesurement':
                ...
                if (value) {
                  ...
                  this.record.mesurement[index].fail = 
                    (min != 0 && Number(value) < Number(min)) || (max != 0 && Number(value) > Number(max)) 
                    ? true : false;
                }
                break;
